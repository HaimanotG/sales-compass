...
async function transition(id, action, body = {}) {
  ...
  if (action === "sent") {
    ...
    if (lead.status === "sent") {
      ...
      // breakup email becomes due a week after follow-up 2
      ...
      // last touch: give the breakup 5 days, then the queue suggests marking it dead
      await run("UPDATE leads SET breakup_sent_at = ?, follow_up_due_at = ? WHERE id = ?",
        [today, addDays(today, 5), id]);
...
async function importCSV(csvText) {
  ...
  if (idx("brand") === -1 || idx("vertical") === -1) {
    throw new ApiError("CSV must have a header row with at least: brand, vertical (plus optional domain, size_estimate, source, founder, contact, comp1, comp2, comp3)");
  }
  // dedupe against existing leads (and within the CSV itself) by brand or url
  const existing = await all("SELECT brand, url FROM leads");
  const seenBrands = new Set(existing.map((l) => normBrand(l.brand)));
  ...
  for (const r of rows.slice(1)) {
    ...
    const url = cell("domain") || cell("url");
    if (seenBrands.has(normBrand(brand)) || (normUrl(url) && seenUrls.has(normUrl(url)))) { duplicates++; continue; }
    seenBrands.add(normBrand(brand));
    ...
    await createLead({
      ...
      competitor_1: cell("competitor_1") || cell("comp1"),
      competitor_2: cell("competitor_2") || cell("comp2"),
      competitor_3: cell("competitor_3") || cell("comp3"),
      status: "new",
    });
    imported++;
  }
