...
async function getSettings() {
  ...
  // Built-in signal variants always reflect the latest code version. Strip any
  // stored template whose name matches a current or deprecated signal name, then
  // append the code-defined set. Custom templates with unrecognised names survive.
  const DEPRECATED_SIGNAL_NAMES = new Set(["price drop / promo", "competitor stockout", "promo / sale launch"]);
  const signalNames = new Set(SIGNAL_TEMPLATES.map((t) => t.name));
  templates = templates.filter((t) => !signalNames.has(t.name) && !DEPRECATED_SIGNAL_NAMES.has(t.name));
  for (const t of SIGNAL_TEMPLATES) {
    templates.push({ name: t.name, text: t.text, subject: t.subject });
  }
  return {
...
const LEAD_FIELDS = [
  ...
  "template_used", "deal_value_mo", "notes", "do_not_email",
];
...
async function createLead(body) {
  ...
  const lead = {
    ...
    do_not_email: body.do_not_email === 1 || body.do_not_email === "1" || body.do_not_email === true ? 1 : 0,
...
async function updateLead(id, body) {
  ...
  for (const f of LEAD_FIELDS) {
    ...
    } else if (f === "competitor_urls_verified" || f === "do_not_email") {
...
// Exported so CLI tooling (scripts/mark-sent.js) drives the exact same sent/follow-up
// cadence and actions streak-log as the UI, instead of duplicating the logic.
export async function transition(id, action, body = {}) {
  const lead = await getLead(id);
  if (!lead) throw new ApiError("lead not found");
