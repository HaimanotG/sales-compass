...
export const SIGNAL_TEMPLATES = [
  {
    ...
    text: [
      "Hi {founder_first},",
      "[hook, rewrite into one human line: {change_competitor} {change_detail}]. That's usually a 3 to 5 day window where their search traffic and social mentions don't have anywhere to go.",
  ...
  },
  {
    ...
    text: [
      "Hi {founder_first},",
      "[hook, rewrite into 2 to 3 lines: this week on {change_competitor}, {change_detail}]",
...
const MIGRATIONS = [
  ...
  // Operator park flag: set when a lead must not be emailed (unverified address, bounced,
  // do-not-contact). The outbound-intelligence skill excludes do_not_email=1 from every
  // candidate query so a parked lead never re-surfaces in a draft batch.
  "ALTER TABLE leads ADD COLUMN do_not_email INTEGER DEFAULT 0",
];
