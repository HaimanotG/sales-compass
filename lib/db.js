...
// the sender always rewrites one human line, but never has to look up data and the
// tool never auto-emits a wrong sentence. change_competitor names the competitor
// that actually moved (the domain at the start of latest_competitor_change), which
// is usually not competitor_1. Empty slots render away cleanly via renderTemplate().
// These are merged in at read time (getSettings) so they appear on existing DBs
// without a reseed, while users can still override a variant by defining one of the
// same name in copy_templates.
export const SIGNAL_TEMPLATES = [
  {
    name: "price drop / promo",
    subject: "{change_competitor} pricing",
    text: [
      "Hi {founder_first}, [hook, rewrite into one human line: {change_competitor} just {change_detail}]. Figured you would want to see it before your customers do.",
  ...
  },
  {
    ...
    subject: "{change_competitor} stockout",
    text: [
      "Hi {founder_first}, [hook, rewrite into one human line: {change_competitor} just {change_detail}]. Usually a short window to catch their demand before they restock.",
  ...
  },
  {
    ...
    subject: "{change_competitor} promo",
    text: [
      "Hi {founder_first}, [hook, rewrite into one human line: {change_competitor} just {change_detail}]. The kind of thing that quietly pulls sales if you don't see it early.",
